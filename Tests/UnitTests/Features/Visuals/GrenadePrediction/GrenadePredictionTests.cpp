...
TEST(GrenadePredictionTest, FirstOverdueNewWeaponCannotStageOldCandidate)
{
    GrenadePredictionState state{};
    const auto oldWeapon = reinterpret_cast<const void*>(1);
    const auto newWeapon = reinterpret_cast<const void*>(2);
...
TEST(GrenadePredictionTest, OwnedCandidateStagesAndFinalizes)
{
    GrenadePredictionState state{};
    const auto weapon = reinterpret_cast<const void*>(1);
    state.tempTrajectory.valid = true;
    ...
    EXPECT_TRUE(state.stageOwnedTempTrajectory(weapon, 7));
    state.finalizeStagedTrajectory(true, true, 10.0f);

    EXPECT_TRUE(state.lastCommittedTrajectory.valid);
    EXPECT_EQ(state.lastCommittedTrajectory.pointsCount, 1);
    EXPECT_TRUE(state.hasCommitCurtime);
    EXPECT_FLOAT_EQ(state.lastCommitCurtime, 10.0f);
}

TEST(GrenadePredictionTest, MissingOrDifferentOwnedCandidateDoesNotReplaceOrTimestampCache)
{
    GrenadePredictionState state{};
    state.lastCommittedTrajectory.valid = true;
    state.lastCommittedTrajectory.pointsCount = 2;
    const auto weapon = reinterpret_cast<const void*>(1);
    state.tempTrajectory.valid = true;
    state.tempTrajectory.pointsCount = 1;
    state.tagTempTrajectory(weapon, 7);

    EXPECT_FALSE(state.stageOwnedTempTrajectory(reinterpret_cast<const void*>(2), 7));
    state.finalizeStagedTrajectory(false, true, 10.0f);

    EXPECT_EQ(state.lastCommittedTrajectory.pointsCount, 2);
    EXPECT_FALSE(state.hasCommitCurtime);
}

TEST(GrenadePredictionTest, SequenceChangeInvalidatesOwnedCandidate)
{
    GrenadePredictionState state{};
    const auto weapon = reinterpret_cast<const void*>(1);
    state.tempTrajectory.valid = true;
    ...
    EXPECT_FALSE(state.ownsTempTrajectory(weapon, 7));
}
