...
public:
    ...
    void handleGrenadePrediction(auto&& playerPawn, auto&& activeWeapon) noexcept
    {
        ...
        if (event) {
            if (eventWeapon != weaponPtr || !eventStrengthValid) {
                finishEvent();
                return;
            }
            finishEvent();
        ...
        if (state.throwObservation.isFinalized()) {
            hideLivePrediction();
            renderLastCommittedTrajectory(hasCurtime, currentCurtime);
            return;
        }
        ...
        const auto eyePos = absOrigin.value() + cs2::Vector{0.0f, 0.0f, eyeHeight};
        const auto viewAngles = eyeAngles.value();
        ...
        simulator.simulate(traj, spawnPos.value(), initialVelocity, kind, static_cast<void*>(playerEntity));
...
private:
    ...
    void hideLivePrediction(bool preserveCandidate = false) noexcept
    {
        auto& state = context().state();
        hideContainerPanel(state.liveContainerPanelHandle);
